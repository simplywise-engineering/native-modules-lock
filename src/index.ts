...
export async function checkLockfile(lockFileName?: string) {
    ...
    const repoDigest = await computeLocks().then(contents => md5(contents.join('\n')));
    const lockFileDigest = await readFile(lockPath).then(buffer => md5(buffer));

    if (repoDigest !== lockFileDigest) {
        throw new Error(`${lockFile} is not synchronized: repo state is ${repoDigest} while lockfile state is ${lockFileDigest}`)
    }
