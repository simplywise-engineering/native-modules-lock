...
export const readFile: (arg1: number | fs.PathLike) => Promise<Buffer> = promisify(fs.readFile);
...
export async function mapFile<T>(path: Path, callback: (buffer: Buffer) => T): Promise<T> {
